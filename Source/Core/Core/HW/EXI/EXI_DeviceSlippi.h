...
private:
  ...
  // CoreTiming event that polls the Melee scene and character-select state once
  // per frame, so menu, character-select and offline-scene presence keep
  // updating even when there's no online frame driving it. Registered when
  // Discord RPC is enabled and cancelled in the destructor.
  CoreTiming::EventType* m_discord_scene_event = nullptr;
  static void DiscordSceneUpdate(Core::System& system, u64 userdata, s64 cycles_late);
